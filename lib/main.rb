...
class NextStopDisplay < CFSM
  ...
  state :departing, :on => :exit_stop_buffer, :transition => :between_stops, :exec => set_next_stop
  ...
  state :between_stops, :on => :stop_button_push, :transition => :stopping_at_nxt_stop do |event, nxt_state|
    DisplayEvent.new :displayText => "Stopping:\n#{@nxtStop.name}"
  end
  
  state :stopping_at_nxt_stop, :on => :enter_stop, :transition => :in_stop_zone do |event, nxt_state|
    DisplayEvent.new :displayText => "Stopping:\n#{@nxtStop.name}"    
  end
  ...
  state :in_stop_zone, :on => :doors_open, :transition => :doors_open do |event, nxt_state|
